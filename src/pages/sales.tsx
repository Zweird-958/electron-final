...
export const SalesPage = () => {
  ...
  const { sales, loading, dateFilter, setDateFilter, exportCsv, exportPdf, getSummary } = useSales()
    ...
    <div className="flex h-full flex-col">
      <div className="flex items-center justify-between border-b border-border px-6 py-4">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="outline" onClick={exportPdf} disabled={sales.length === 0}>
      ...
      <div className="flex-1 overflow-auto px-6 py-4">
        {loading ? (
          ...
          <p className="py-12 text-center text-sm text-muted-foreground">{t('sales.empty')}</p>
        ) : (
          <Table>
            <TableHeader>
              <TableRow>
            ...
            <TableBody>
              {sales.map((sale) => (
                <TableRow key={sale.id}>
                  ...
                  <TableCell>
                    <Button variant="ghost" size="icon-xs" onClick={() => openDetail(sale.id)}>
                      <Eye />
                    </Button>
                  </TableCell>
                </TableRow>
